...
function openLink(url)
    pcall(gta._Z12AND_OpenLinkPKc, url)
end
...
-- =========================================================
-- WEAPON EXCLUSION (SNIPER)
-- Rifle (Country Rifle) = 33, Sniper Rifle = 34
-- Crosshair kita disable buat weapon ini karena udah ada scope sendiri
-- =========================================================
local SNIPER_WEAPONS = { [33] = true, [34] = true }
local function isSniperEquipped()
    local w = getCurrentCharWeapon(PLAYER_PED)
    return SNIPER_WEAPONS[w] == true
end
...
loadTextures = function()
    ...
    local ok, p = pcall(io.popen, 'dir "'..dir..'" /b 2>nul || ls "'..dir..'" 2>/dev/null')
    if ok and p then
...
-- =========================================================
-- loadConfig: dibersihkan dari triple-duplicate block lama
-- (itu penyebab lag & bug index geser pas load config)
-- =========================================================
local function loadConfig()
    ...
    -- MODE SWITCH (ini juga yang ngisi textures + apply nama tersimpan)
    if siteM16[0] then
        mode_full[0] = true
        mode_single[0] = false
        switchMode(false)
    ...
    -- apply texture berdasarkan nama yang tersimpan (kalau ketemu)
...
function main()
    ...
    introActive = true
    window[0] = true
    sampRegisterChatCommand("ccross", function()
...
-- =========================================================
-- OPTIMASI: dulu loop ini full-scan SEMUA ped x SEMUA bone
-- SETIAP FRAME kalau redOnTarget aktif -> ini paling berat.
-- Sekarang: throttle 1 dari 3 frame, dan stop begitu jarak makin membesar
-- (bone diurutkan kasar biar early-exit lebih sering kena)
-- =========================================================
local fovCheckCounter = 0
local cachedIsTarget = false
...
imgui.OnFrame(function()
    ...
    return texture ~= nil
        ...
        and not paused
        and not notSpawned
        and not isSniperEquipped()   -- <== crosshair off buat sniper/rifle
end, function()
    ...
    if redOnTarget[0] and isPlayerAiming() then
        -- THROTTLE: cek FOV cuma 1 dari 3 frame, sisanya pakai hasil cache
        fovCheckCounter = fovCheckCounter + 1
        if fovCheckCounter >= 3 then
            fovCheckCounter = 0
            local ok, result = pcall(isTargetInFov, centerX, centerY, DATA.raioFov)
            cachedIsTarget = ok and result or false
        end
        isTarget = cachedIsTarget
    else
        fovCheckCounter = 0
...
-- =========================================================
-- LAYOUT: dihitung sekali (bukan tiap frame window dibuka),
-- dan diclamp ke DisplaySize biar aman di HP layar kecil / notch
-- =========================================================
local childW, childH, winW, winH
...
local function computeLayout()
    ...
    local screenW = io_.DisplaySize.x
    local screenH = io_.DisplaySize.y

    if screenW > 0 and winW > screenW * 0.95 then
        local scaleFix = (screenW * 0.95) / winW
        winW = winW * scaleFix
        childW = childW * scaleFix
    end

    if screenH > 0 and winH > screenH * 0.95 then
        winH = screenH * 0.95
        childH = math.max(150 * MDS, winH - 60 * MDS)
    end
end
...
imgui.OnInitialize(function()
    ...
    if doesFileExist(FONT_PATH) then
        fontTitle      = io_.Fonts:AddFontFromFileTTF(FONT_PATH, 18)
        fontTitleSmall = io_.Fonts:AddFontFromFileTTF(FONT_PATH, 11)
    end

    local iconRanges = imgui.new.ImWchar[3](faicons.min_range, faicons.max_range, 0)
    fontAwesome = io_.Fonts:AddFontFromMemoryCompressedBase85TTF(
...
function imgui.BeginCustomTitle(title, titleSizeY, var, flags)
    ...
    if opened then
        ...
        for _, offset in ipairs(textOffsets) do
            dl:AddText(
...
imgui.OnFrame(function()
    ...
    -- clamp posisi window biar ga keluar layar di HP mana pun
    local io_ = imgui.GetIO()
    local screenW = io_.DisplaySize.x
    local screenH = io_.DisplaySize.y
